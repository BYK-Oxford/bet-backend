...
class CurrentLeagueService:
    ...
    def create_current_league(self, league_data: dict):
        ...
        team = self.team_service.get_or_create_team(league_data.get("team"), league_data.get("league_code"))
        league = self.league_service.get_or_create_league(league_data.get("league_code"))
        season = self.season_service.get_or_create_season(league_data.get("season"))
        
        # Check if standing already exists for this team in this league and season
        existing_standing = self.db.query(CurrentLeague).filter_by(
        ...
        if existing_standing:
            return existing_standing  # Prevent duplicate standings
